...
Lottery.prototype = {
    ...
    _drawText: function(){
        ...
        var scale = this.options.scale;
        for(var i = 0; i < this.size; i++){
			if(this.options.font.lay == "landscape"){
				...
				var textStr  = this.options.products[i].name;
				var z = 0;//y轴偏移量系数
				for(var j=textStr.length-1;j>=0; j--){
					...
					this.ctx.fillStyle = this.isArray(fonts.color) ? fonts.color[i%fonts.color.length] : fonts.color;
					this.ctx.font = this._getFontStyle();
					this.ctx.fillText(textStr[j], -this.ctx.measureText(textStr[j]).width/2, -(fonts.y/1.25+30*z));
					this.ctx.restore();	
					z++;
				}
			}
    ...
    /*开始旋转转盘 isRotate是否旋转*/
    _beginRotate: function(isRotate=true){
    ...
    _setOptions: function(options){
        ...
        var config = {
			...
			/*点击抽奖的转动*/
            handlerRotate: function(_interface){},
            outerRadius: '',
            innerRadius: 0,
            /*循环填充数组颜色*/
